...
query = dedent(
    ...
    base_data AS (
        ...
        WHERE
            ...
            AND b.created_at >= DATEADD(hour, -8, DATEADD(day, -p.return_user_days, p.start_date))
            AND b.created_at < DATEADD(hour, -8, DATEADD(day, p.retention_days, p.end_date))
    ),
    ...
    stats_by_daily_group AS (
        SELECT
            ...
            COUNT(DISTINCT
                CASE
                    WHEN
                        DATEDIFF('day', u.bj_date_last_bet, u.bj_date_raw) > p.return_user_days
                    THEN u.user_id
                END
            ) AS num_return_users,
