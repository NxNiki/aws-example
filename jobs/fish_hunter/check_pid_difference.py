...
if __name__ == "__main__":
    ...
    data_loader = DataLoader(
        backend=AthenaBackend(
            database="agfish",
            output_location=f"s3://{S3_BUCKET}/ds-data-fish_hunter/product_id_pa",
            ctas_approach=True,  # Use CTAS to avoid 32MB row size limit
        )
    )
    ...
    file_path = f"{LOCAL_ROOT}/jobs/output_fish_hunter/product_id.parquet"
    df = redshift_loader.query_to_df(query=query, local_cache=file_path, reload=True)
    print(df)

    file_path = f"{LOCAL_ROOT}/jobs/output_fish_hunter/product_id_pa.parquet"
