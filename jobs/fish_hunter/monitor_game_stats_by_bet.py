...
def create_dashboard(csv_path):
    ...
    app.layout = html.Div(
        [
            ...
            html.Div(
                [dcc.Graph(id="combined-plot", style={"height": "1550px"})],  # increase figure height
                style={"width": "80%", "height": "95%", "display": "inline-block"},
            ),
        ]
    )
    ...
    def update_plot(session, strat_a, strat_b, strat_c, sel_metrics, log_check, thresh):
        ...
        fig = make_subplots(rows=3, cols=1, shared_xaxes=True, subplot_titles=(strat_a, strat_b, strat_c))
        ...
        fig.update_xaxes(title_text="bet_index", rangeslider=dict(visible=True, thickness=0.025))  # thinner slider
