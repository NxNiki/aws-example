"""
-- sister of etl_feature_engineer.py with two differences:
--   1. Includes ALL ab_partition groups (AI, AB_TEST_A, AB_TEST_B, Default) instead of
--      restricting to Default. The bucket is exposed as `ai_group` (same convention as
--      etl_game_stats_daily_by_user_group.py).
--   2. Does NOT drop incomplete tail agg_groups -- the final group in each session is
--      kept even if it has fewer than SESSION_LENGTH rows.

-- See etl_feature_engineer.py for the full description of derived metrics, streaks,
-- aggregations, and per-feature normalization. The CTE structure is identical.
"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="ETL Feature Engineer for SS03 Mahjiang Streak (full data)")
    parser.add_argument(
        "--bastion-ip",
        type=str,
