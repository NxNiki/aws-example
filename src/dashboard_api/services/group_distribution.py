...
from dashboards.user_stats_aggregates import RETENTION_LOAD_EXTRA_DAYS, DataMetrics, _bootstrap_ci
...
def load_group_distribution(
    ...
    lo_bound = clip_min if clip_min is not None else -np.inf
    hi_bound = clip_max if clip_max is not None else np.inf
