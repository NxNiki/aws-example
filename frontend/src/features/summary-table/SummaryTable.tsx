...
import type { SummaryStat } from "../../api/types";
...
export function SummaryTable() {
  ...
  const c = s.controls.summaryTable; // this tab's own cohorts; dates/granularity are global
  const dg = s.dateGroups;
  const t = s.summaryTable;
  const groups = s.config?.groups ?? [];

  // Refetch on config / date-group / cohort / per-metric option / p-values
  // changes. Metric selection, stat selection and the reference column are
