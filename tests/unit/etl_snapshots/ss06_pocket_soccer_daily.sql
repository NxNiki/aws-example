...
user_mathtable_last_spin_ai AS (
    ...
    GROUP BY t.activity_date, t.user_id, t.mathtable
),
...
user_remaining_bet AS (
    ...
    LEFT JOIN user_bets AS b
        ...
        AND b.activity_date = m.activity_date
        AND b.spin_id > m.mathtable_last_spin_id
        AND b.ab_group_id = 'jojpin-9mokha-rexQug'
    GROUP BY m.activity_date, m.user_id, m.mathtable
),
...
SELECT
    ...
    CASE
        WHEN DATEDIFF('day', fb.first_bet_date, us.activity_date) <= 3 THEN 'new'
        WHEN DATEDIFF('day', fb.first_bet_date, us.activity_date) <= 7 THEN 'beginner'
        ELSE 'old'
    END AS user_group,
...
LEFT JOIN user_remaining_bet AS rb
    ...
    AND rb.activity_date = us.activity_date
