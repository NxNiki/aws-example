...
WHERE
    t.game_id = 'SS03'
    AND CONVERT_TIMEZONE('UTC', 'Asia/Shanghai', t.created_at) >= '2024-12-30'
...
user_stats AS (
    ...
    GROUP BY t.activity_week, t.ai_group, t.user_id
),
...
user_mathtable_last_spin_ai AS (
    ...
    GROUP BY t.activity_week, t.user_id, t.mathtable
),
...
user_remaining_bet AS (
    ...
    LEFT JOIN user_bets AS b
        ...
        AND b.activity_week = m.activity_week
        AND b.spin_id > m.mathtable_last_spin_id
        AND b.ab_group_id = 'jojpin-9mokha-rexQug'
    GROUP BY m.activity_week, m.user_id, m.mathtable
),
...
SELECT
    us.activity_week AS activity_date,
    us.ai_group,
    us.user_id,
    us.user_mathtable_change,
    CASE
        WHEN DATEDIFF('day', fb.first_bet_date, us.activity_week) <= 3 THEN 'new'
        WHEN DATEDIFF('day', fb.first_bet_date, us.activity_week) <= 7 THEN 'beginner'
        ELSE 'old'
    END AS user_group,
...
LEFT JOIN user_remaining_bet AS rb
    ...
    AND rb.activity_week = us.activity_week
    AND rb.ai_group = us.ai_group
WHERE us.activity_week >= '2024-12-30'
ORDER BY us.activity_week DESC, us.ai_group DESC, us.user_id DESC;
