...
binned AS (
    SELECT
        t.*,
        ROUND((t.session_bet_index - 1) / 40) AS agg_group
    FROM raw_stats AS t
),

perc_delta_t AS (
    SELECT
        user_id, ai_group, session_start_date, session_group, agg_group,
    ...
    FROM binned
    GROUP BY user_id, ai_group, session_start_date, session_group, agg_group
),
perc_delta_t_ng AS (
    SELECT
        user_id, ai_group, session_start_date, session_group, agg_group,
