...
# run bash infra/sagemaker/build.sh from project root directory to ensure build context is correctly specified.
...
docker build --platform linux/amd64 -t $IMAGE_NAME -f infra/sagemaker/Dockerfile ./
