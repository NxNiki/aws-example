...
#   2. Create task definition (replace ACCOUNT_ID and ECR_IMAGE_URI in infra/ecs_etl_task_def.json).
...
sed -e "s|ACCOUNT_ID|$ACCOUNT_ID|g" -e "s|ECR_IMAGE_URI|$ECR_IMAGE|g" infra/ecs_etl_task_def.json > "$TASK_DEF_FILE"
