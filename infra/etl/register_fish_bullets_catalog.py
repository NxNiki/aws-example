"""Register the Athena table ``bituslabs_ds.fish_bullets_group_tag``.

Points the Glue catalog at the ``output_fish_bullets_group_tag/bullets``
dataset (FM01 bullets + the policy-correct ``group_tag`` column — see
``jobs/etl/sagemaker/fish_hunter/etl_fish_bullets_group_tag.py``), the same
external-parquet-table pattern as ``bituslabs_ds.slot_orders_ab_group``.
Column types are read from an actual parquet footer, so run this AFTER the
first backfill. Partition projection (period date) means no crawler and no
MSCK — new partitions are queryable the moment they land.

Usage:
    poetry run python infra/etl/register_fish_bullets_catalog.py
"""
...
def main() -> None:
    table_input: dict[str, Any] = {
        ...
        "Parameters": {
            ...
            "storage.location.template": LOCATION + "period=${period}",
        },
        "PartitionKeys": [
